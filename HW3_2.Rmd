...
It appears that there are more females than males that have more than high school education and less than high school education while, there are more males who have high school equivalent education than females. 
...
This matches our previous graph. There is more density around age 30 for education more than high school.
...
merged_df_totalact <-
  ...
  group_by(seqn, sex, education, age, min) |>
...
ggplot(merged_df_totalact,
       aes(x = min, y = total_activity, color = sex, fill = sex)) +
  geom_point(alpha = .5) +
  geom_smooth(se = FALSE) + 
  facet_grid(. ~ education) 
```

Data appears off and unable to troubleshoot. 
However we would expect the most amount of activity during day time hours (maybe after 7am), and activity decreasing in the evening (after 9pm). 
...
This dataset has information from January 2020, July 2020, January 2024, and July 2024.

There are `r nrow(citibike_tidy)` rows and `r ncol(citibike_tidy)` columns in our combined dataset. 

Variables include ride_id(chr), rideable_type(chr), weekdays(chr), duration(dbl), start_station_name(chr), end_station_name (chr), member_casual (chr),
month (chr), year (chr), and month_year (chr)
...
It appears that overall, the number of people using citi bike has grown from January 2020 to July 2024. Summer months seem to have more frequent rides than winter months and there are more members than casual riders in each month_year. 
...
From our output, Pier 61 at Chelsea Piers had the highest count of number of 
rides originating from this station in July 2024. 
...
Plot days of the week on x-axis and median ride duration on y? 

```{r}
citibike_variation <- citibike_tidy |>
  group_by(month, year, weekdays) |>
  summarize(median_duration = median(duration, na.rm = TRUE)) |>
  ungroup()

ggplot(citibike_variation,
    aes(x = weekdays, y = median_duration, fill = month)) +
      geom_bar(stat = "identity", position = "dodge") +
      facet_grid(. ~ year)
  
```

There were overall longer duration rides in 2020 compared to 2024. Across both 
years, the summer month July has higher mediation duration rides than the 
winter month January. There is wider variation in 2020 between January and 
...
```{r}
citibike_dist <- citibike_tidy |>
  filter(year == 2024)

ggplot(citibike_dist, aes(x = member_casual, y = duration, fill = rideable_type)) +
  geom_boxplot() +
  facet_grid(month ~ rideable_type) 
```

Casual members appear to have higher median ride durations than members do 
across both monthy and rideable_type. January tends to have short ride 
durations than July does, which makes sense since it is easier and more
enjoyable to bike in the summer. 
ride in the summer 